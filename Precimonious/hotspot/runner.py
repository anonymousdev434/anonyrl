...
def run_config(search_config, original_config, benchmark):
  ...
  if os.path.exists(TO_RUN):
    ...
    try:
      ...
      process.kill()
    os.system(f"../../utilities/quality -a accurate_output -t test_output -m MAE -e {EPSILON}")
...
def run_orig_config(search_config, original_config, benchmark):
  ...
  if os.path.exists(TO_RUN):
    ...
    try:
      ...
      print('Timed out - killing', process.pid)
      process.kill()
    os.system(f"../../utilities/quality -a accurate_output -t accurate_output -m MAE -e {EPSILON}")
