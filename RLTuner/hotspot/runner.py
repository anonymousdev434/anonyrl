...
PROB_SIZE3 = "100"
PROB_SIZE4 = "4"
PROB_SIZE5 = "../tempscripts/input.bin"
...
def setup(benchm, time_out, epsilon, class_name, b_epsilon):
    ...
    TO_RUN = "../tempscripts/hotspot"
    benchfile = "hotspot_openmp.cpp"
...
def run_config(state_num):
    ...
    if os.path.exists(TO_RUN):
        print("Round 1:")
        process = subprocess.Popen([TO_RUN, PROB_SIZE1, PROB_SIZE2, PROB_SIZE3, PROB_SIZE4, PROB_SIZE5, "test_output"])
        try:
...
def run_orig_config(original_config):
    ...
    if os.path.exists(TO_RUN):
        print("Round 1:")
        process = subprocess.Popen([TO_RUN, PROB_SIZE1, PROB_SIZE2, PROB_SIZE3, PROB_SIZE4, PROB_SIZE5, "accurate_output"])
        try:
            process.wait(timeout=TIME_OUT)
        except subprocess.TimeoutExpired:
            print('Timed out - killing', process.pid)
