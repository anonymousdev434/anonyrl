...
PROB_SIZE1 = "../tempscripts/fvcorr.domn.097K"

original_runtime = 0
df = pd.DataFrame()

def setup(benchm, time_out, epsilon, class_name, b_epsilon):
    global benchmark, TIME_OUT, EPSILON, RESULTSDIR, TO_RUN, benchfile, CLASS, B_EPSILON
    benchmark = benchm
    TIME_OUT = time_out
    EPSILON = epsilon
    CLASS = class_name
    ...
    TO_RUN = "../tempscripts/euler3d"
    benchfile = "pre_euler3d_cpu.cpp"

    return RESULTSDIR


def run_config(state_num):
    ...
    config_filename = f"updated_config.json"
    ...
    os.system("python3 recast-args.py config_temp.json")
    ...
    if os.path.exists(TO_RUN):
        print("Round 1:")
        process = subprocess.Popen([TO_RUN, PROB_SIZE1, "test_output"])
        try:
            process.wait(timeout=TIME_OUT)
        except subprocess.TimeoutExpired:
            print('Timed out - killing', process.pid)
            process.kill()
        os.system(f"../../utilities/quality -a accurate_output -t test_output -m MAE -e {B_EPSILON}")
    ...
    if result == 1:
        ...
        print("mv config_temp.json {}/{}.json".format(RESULTSDIR, "VALID_config_" + benchmark + "_" + str(state_num)))
        os.system("mv config_temp.json {}/{}.json".format(RESULTSDIR, "VALID_config_" + benchmark + "_" + str(state_num)))
        ...
        print("mv config_temp.json {}/{}.json".format(RESULTSDIR, "INVALID_config_" + benchmark + "_" + str(state_num)))
        os.system("mv config_temp.json {}/{}.json".format(RESULTSDIR, "INVALID_config_" + benchmark + "_" + str(state_num)))
    elif result == -1:
        config_info_dict["label"] = "FAIL"
        print("mv config_temp.json {}/{}.json".format(RESULTSDIR, "FAIL_config_" + benchmark + "_" + str(state_num)))
        os.system("mv config_temp.json {}/{}.json".format(RESULTSDIR, "FAIL_config_" + benchmark + "_" + str(state_num)))
...
def run_orig_config(original_config):
    ...
    logging.info("-------- running config {} --------".format(json_string))
    ...
    if os.path.exists(TO_RUN):
        print("Round 1:")
        process = subprocess.Popen([TO_RUN, PROB_SIZE1, "accurate_output"])
        try:
            process.wait(timeout=TIME_OUT)
        except subprocess.TimeoutExpired:
            print('Timed out - killing', process.pid)
            process.kill()
        os.system(f"../../utilities/quality -a accurate_output -t accurate_output -m MAE -e {B_EPSILON}")
    ...
    if result == 1:
        config_info_dict["label"] = "VALID"
        print("mv config_temp.json {}/{}.json".format(RESULTSDIR, "VALID_orig_config_" + benchmark))
        os.system("mv config_temp.json {}/{}.json".format(RESULTSDIR, "VALID_orig_config_" + benchmark))
        # run EXE_COUNT times in total to get average runtime
        avg_runtime = 0
        avg_runtime += runtime
        for i in range(EXE_COUNT - 1):
            if os.path.exists(TO_RUN):
                print("Round {}:".format(i+2))
                process = subprocess.Popen([TO_RUN, "-p"])
                try:
                    process.wait(timeout=TIME_OUT)
                except subprocess.TimeoutExpired:
                    print('Timed out - killing', process.pid)
        ...
        config_info_dict["label"] = "INVALID"
        print("mv config_temp.json {}/{}.json".format(RESULTSDIR, "INVALID_orig_config_" + benchmark))
        os.system("mv config_temp.json {}/{}.json".format(RESULTSDIR, "INVALID_orig_config_" + benchmark))
    elif result == -1:
        config_info_dict["label"] = "FAIL"
        print("mv config_temp.json {}/{}.json".format(RESULTSDIR, "FAIL_orig_config_" + benchmark))
        os.system("mv config_temp.json {}/{}.json".format(RESULTSDIR, "FAIL_orig_config_" + benchmark))
