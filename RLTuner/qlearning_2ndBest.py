...
def main():
    ...
    try:
        ...
        for episode in range(30):            
            ...
            second_best_speedup = None
            for item in state_config_map.values():
                if item.result == 1:
                    speedup = item.speedup

                    if best_speedup is None or speedup > best_speedup:
                        second_best_speedup = best_speedup
                        best_speedup = speedup
                    elif (second_best_speedup is None or speedup > second_best_speedup) and speedup != best_speedup:
                        second_best_speedup = speedup

            if second_best_speedup:
                second_best_speedups_state = [x.state for x in state_config_map.values() if x.result == 1 and x.speedup == second_best_speedup][0]
                second_best_speedup_config = [x.config for x in state_config_map.values() if x.result == 1 and x.speedup == second_best_speedup][0]
                second_best_speedup_searched = [x.searched for x in state_config_map.values() if x.result == 1 and x.speedup == second_best_speedup][0]
                logging.info(f"second_best_speedups_state: {second_best_speedups_state}, second_best_speedup: {second_best_speedup}")
                logging.info(f"second_best_speedup_searched: {second_best_speedup_searched}")
            ...
            if best_speedup:
                ...
                elif best_speedup_searched and second_best_speedup and not second_best_speedup_searched:
                    S = second_best_speedups_state

                    second_best_index = [key for key, value in state_config_map.items() if value.speedup == second_best_speedup and value.result == 1][0]
                    state_config_map[second_best_index] = state_config_map[second_best_index]._replace(searched=True)

                    tmp_config = utilities.convert_format(second_best_speedup_config)

                    print(f"Set the state to the second best speedup state: {S}")
                    logging.info(f"Set the state to the second best speedup state: {S}")
                else:
