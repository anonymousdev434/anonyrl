...
int main(int argc, char *argv[]){
    ...
    while ((c = getopt (argc, argv, "a:t:m:e:h")) != -1) {
        switch (c){
            case 'a':
                accurate = optarg;
                break;
    ...
    if ( !checkMetrics(metric) ){
        printf("We do not support this metric %s\nSupported Metrics:\n");
    ...
    double quality = 0.0;
    if ( type == DOUBLE){
        quality = computeQuality((double *) acc,(double *)tst, numElements, metric, atoi(epsilon));
    }
    else if ( type == INT){
        quality = computeQuality( (int *) acc, (int *) tst, numElements, metric, atoi(epsilon));
    }
    else if ( type == -1 ){
        quality=INFINITY;
    }
