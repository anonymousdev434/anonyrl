...
int StrToInt(char *token, IntT *retVal)
{
   char *c ;
   char *endptr ;
   int decimal_base = 10 ;
...
static void ParseError(char *message, int myRank)
{
   if (myRank == 0) {
      printf("%s\n", message);
#if USE_MPI      
                               ...
                               Int_t numRanks)
{
   ...
   std::cout << "   Error threshold: " << std::setw(12) << epsilon << "\n";
   if (MaxAbsDiff && TotalAbsDiff && MaxRelDiff && (MaxAbsDiff <= epsilon)) {
   // if (MaxAbsDiff <= epsilon) {
      std::cout << "   Error threshold check: Successful\n";
   } 
   else {
      std::cout << "   Error threshold check: Unsuccessful\n";
   }
   ...
   fputs((MaxAbsDiff && TotalAbsDiff && MaxRelDiff && (MaxAbsDiff <= epsilon)) ? "true\n" : "false\n", fp_log);
   fprintf(fp_log, "%20.13E\n", MaxAbsDiff);
   printf("   MaxAbsDiff           = %20.13E\n", MaxAbsDiff);
