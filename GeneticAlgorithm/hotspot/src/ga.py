...
def search_elite(results, runtimes, pop):
    ...
    elite_indices = [i for i, (result, _) in enumerate(results, start=1) if result == 1]

    if elite_indices:
        elite_index = min(elite_indices, key=lambda i: runtimes[i - 1])
        if elite_index > 0:
            elite_individual = pop[elite_index-1]
            elite_runtime = runtimes[elite_index-1]
            elite_return_index = elite_index - 1
            print(f"Elite_individual, Config {elite_index} Runtime: {runtimes[elite_index - 1]} Result: {results[elite_index - 1][0]}")
            logging.info(f"Elite_individual, Config {elite_index} Runtime: {runtimes[elite_index - 1]} Result: {results[elite_index - 1][0]}")
        else:
            elite_individual = None
